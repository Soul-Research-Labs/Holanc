...
/// Withdraw circuit (2-in-2-out + exit value).
///
/// Same as Transfer, but with an additional public `exit_value` that
/// represents the amount being withdrawn from the pool to a public address.
///
/// Value conservation:
///   sum(inputs) == sum(outputs) + exit_value + fee
///
/// Public inputs:
///   - merkle_root
///   - nullifiers[2]
///   - output_commitments[2]  (change outputs)
///   - exit_value
///   - fee
template Withdraw(tree_depth) {
    ...
    // ---------------------------------------------------------------
    // Step 1: Verify input notes exist in tree + derive nullifiers
    // ---------------------------------------------------------------
    component in_cm[2];
    component in_nf[2];
...
component main {public [merkle_root, nullifiers, output_commitments, exit_value, fee]} = Withdraw(20);
