...
/// Variable I/O withdraw circuit (4-in, 4-out + exit_value).
///
/// Extends TransferN with a public exit_value for on-chain token release.
///
/// Value conservation: sum(effective_in) == sum(effective_out) + exit_value + fee
template WithdrawN(n_in, n_out, tree_depth) {
    // ── Public inputs ──────────────────────────────────────────────
...
component main {public [merkle_root, nullifiers, output_commitments, exit_value, fee]} = WithdrawN(4, 4, 20);
